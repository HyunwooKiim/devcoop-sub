...
객체간의 관계에 초점이 맞춰져 있어 하위 수준의 도메인을 보고서는 도메인의 전반적인 흐름이나 그 속에서의
주요 관계를 파악하기 어렵다.
...
그것을 **애그리거트 루트**라고 부른다.
애그리거트 루트는 그 애그리거트를 대표하고, 외부에서 접근 가능한 유일한 객체이다.(오로지 루트를 통해야만 내부 객체 접근 가능)

예를 들어 아래와 같이 애그리거트로 묶어두었다고 보자
...
게시글의 규칙은 Post가 관리해야한다. (게시글이 삭제되면 댓글도 삭제, 작성자만 삭제 가능) -> 이 규칙을 Comment가 알아선 안된다.
...
위와 같이 설계 한다면, 다른 게시글로 댓글 이동이 가능하며, 게시글의 상태를 무시할수 있다.(검증 X)
...
```java
    ...
    post.delete();
```
하지만 위와 같이 설계한다면, Comment는 외부에서 직접 수정하지 못하며
반드시 Post를 통해서만 변경해야 하기 때문에 일관성 있게 관리 할 수 있다(규칙을 깨지 않음).

그리고 또한 애그리거트끼리 연결 하때는 직접 객체를 참조하면 안되며 Id만 저장 하는듯이
느슨하게 연결해야한다.

왜냐하면,
1. 경계가 무너진다.
   - Post안에서 User의 상태까지 막 바꾸게 되면 Post가 User까지 관리하게 되는 꼴이된다.
2. 트랜잭션이 커진다.
   - Post를 저장하려다 User도 바꾸고, 다른 것들도 바꾸다 보면 한번의 작업의 범위가 커져 복잡 + 충돌 가능성이 증가한다.
